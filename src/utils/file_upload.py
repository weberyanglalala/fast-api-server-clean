...
async def upload_file_to_r2(file_name: str, file_content: bytes, content_type: str) -> str:
    ...
    async with session.create_client(
        ...
        config=AioConfig(max_pool_connections=10)  # Optional: tune connection pool
    ) as s3_client:
        try:
            # Use await for async put_object
